...
export const updateOrder = createAsyncThunk(
  'orders/updateOrder',
  async ({ orderId, updates }: { orderId: string; updates: Partial<Order> }, { rejectWithValue }) => {
    try {
      const response = await axios.put<Order>(`/api/orders/${orderId}`, updates);
      return response.data;
    } catch (error: any) {
...
export const processOrderPayment = createAsyncThunk(
  ...
  async ({ orderId, isPaid = true }: { orderId: string; isPaid?: boolean }, { rejectWithValue }) => {
    try {
      // Update the order with new payment status
      const response = await axios.put<Order>(`/api/orders/${orderId}/payment`, { isPaid });
      return response.data;
    } catch (error: any) {
...
export const addItemToOrder = createAsyncThunk(
  ...
  async ({ 
    ...
    try {
      const response = await axios.post<Order>(`/api/orders/${orderId}/items`, item);
      return response.data;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || 'Failed to add item to order');
    }
  }
);
...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    clearOperationErrors: (state, action: PayloadAction<OrderOperation>) => {
      state.errors = state.errors.filter(error => error.operation !== action.payload);
    },
    // Helper to clear errors for a specific operation on a specific order
    clearOrderOperationErrors: (state, action: PayloadAction<{operation: OrderOperation, orderId: string}>) => {
      const { operation, orderId } = action.payload;
      state.errors = state.errors.filter(error => 
        !(error.operation === operation && error.id === orderId)
      );
    },
    togglePaymentStatus: (state, action: PayloadAction<string>) => {
      // Find the order by ID
      const orderToUpdate = state.orders.find(order => order._id === action.payload);
      if (orderToUpdate) {
    ...
    setLoadingState: (state, action: PayloadAction<{operation: OrderOperation, isLoading: boolean}>) => {
      const { operation, isLoading } = action.payload;
      state.loadingStates[operation] = isLoading;
    },
    // Record operation success
    recordOperationSuccess: (state, action: PayloadAction<{operation: OrderOperation, id?: string}>) => {
    ...
    recordOperationFailure: (state, action: PayloadAction<{operation: OrderOperation, id?: string, error: string}>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchOrders.pending, (state) => {
        ...
        state.errors = state.errors.filter(error => error.operation !== 'fetch');
      })
      ...
      .addCase(createOrder.pending, (state) => {
        ...
        state.errors = state.errors.filter(error => error.operation !== 'create');
      })
      ...
      .addCase(updateOrder.pending, (state, action) => {
        ...
        if (orderId) {
          state.errors = state.errors.filter(error => 
            !(error.operation === 'update' && error.id === orderId)
          );
        }
      })
      ...
      .addCase(processOrderPayment.pending, (state, action) => {
        ...
        const orderId = action.meta.arg.orderId;
        state.errors = state.errors.filter(error => 
          !(error.operation === 'payment' && error.id === orderId)
        );
      })
      ...
      .addCase(deleteOrder.pending, (state, action) => {
        ...
        state.errors = state.errors.filter(error => 
          !(error.operation === 'delete' && error.id === orderId)
        );
      })
      ...
      .addCase(addItemToOrder.pending, (state, action) => {
        ...
        const orderId = action.meta.arg.orderId;
        state.errors = state.errors.filter(error => 
          !(error.operation === 'addItem' && error.id === orderId)
        );
      })
